...
namespace BenchmarkDotNet.Tests
{
    public class ConfigParserTests
    {
        ...
        {
            var config = ConfigParser.Parse(new[] { "--runtimes", "net462", "MONO", "netcoreapp3.1", "nativeaot6.0", "nativeAOT7.0", "nativeAOT8.0" }, new OutputLogger(Output)).config;
            ...
            Assert.Single(config.GetJobs().Where(job => job.Environment.Runtime is CoreRuntime coreRuntime && coreRuntime.MsBuildMoniker == "netcoreapp3.1" && coreRuntime.RuntimeMoniker == RuntimeMoniker.NetCoreApp31));
